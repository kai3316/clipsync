...
- **AI 工具配置预置** — 默认监视 Claude Code / Codex / Cursor / Gemini CLI 的真实配置路径（CLAUDE.md、settings.json、skills、rules、GEMINI.md 等），一键添加；凭据文件（如 auth.json）默认排除。
- **新旧对比徽标** — 跨设备浏览时每行标注 缺失 / 相同 / 本地新 / 远端新（按哈希与修改时间比对），拉取前即可判断方向。
- **本机配置管理器** — 不配对也能管理监视清单内文件：浏览/预览/编辑保存（自动留 .bak）、删除进回收站（不硬删）、打开所在文件夹；列表按目录树展示，技能/命令等子文件夹可展开折叠，双击直接打开该文件夹。

...
- **直接送达，不弹确认框** —— 同一局域网内的设备都按可信处理，知道配对码的设备更可信任：会话一开就能说话，文件直接收进接收目录，不逐个等你点「接受」
- 代价是对方的设备名与短指纹随邀请立即到达（以前要等你接受）：需要核对身份时，用短指纹线下比对
- **想改回逐个确认** —— 设置 → 局域网发现 → 关掉「任何人可直接发来消息和文件」，之后每条会话与每个文件都要你先点「接受」，对方的短指纹先于内容到达，供你核对
- 文字与文件都能发，文件复用分块传输通道，断线自动重连
- 收到的东西只落在接收目录里：文件名消毒、路径穿越拦截，加上大小、速率与并发上限——在你不掌控的网络上（比如公共 Wi-Fi），这些是唯一的兜底
...
每台设备首次启动时生成一对 Ed25519 密钥，公钥即设备身份。第一次与另一台设备建立连接时，两端显示同一个 8 位配对码（由双方证书指纹按固定顺序拼接后散列得出，两台设备算出的数字因而一致），用户确认后对方证书指纹被锁定；此后指纹变化即告警。线缆上双重加密：TLS 1.3 提供传输层安全，每个帧体再独立做一次 AES-256-GCM。落盘的私钥与剪贴板历史同样是 AES-256-GCM。
...
npm run test:web                   # 旧版网页面板（手机伴侣）前端测试

...
- **互联网配对管理（设备页）** — 从未见过面的设备用 12 位配对码互认；已配对设备在「设备」页统一管理：别名重命名、在线/最近同步状态、单侧取消配对。
- **聊天走互联网** — 与异网设备也能聊天（文本/输入状态/会话状态经中继），局域网优先投递、双通道去重。
- **设备卡聊天入口** — 「设备」页每个在线/已配对设备卡片都有 💬 聊天按钮，右键菜单也有「打开聊天」，一键进入与该设备的会话。
- **送达确认 + 离线补发** — 中继消息带回执（气泡 ✓已送达 / ✗未送达）；对方或中继离线时自动入本地队列，恢复后补发（最多 5 次）。
- 注意：免费公共中继属公共服务，无 SLA；大文件传输与聊天文件仍走局域网直连，不经过中继。

### AI 配置同步

- **统一「配置」tab，跨设备为主角** — 顶部设备条（本机 + 各已配对设备，每台标注差异总数），中部所选设备的清单按工具档案分组，本机管理作为「本机」子视图保留。
- **AI 工具档案取代原始路径列表** — 内置 Claude Code（CLAUDE.md / settings.json / skills）、Codex（config.toml）、Cursor（rules / commands）、Gemini（settings.json / GEMINI.md）四套档案，设置里勾选启用的工具 + 自定义路径兜底；旧版路径列表配置自动迁移。凭据文件（如 auth.json）默认排除。
- **清单三选一落地，绝不静默覆盖** — 每次拉取时选择：覆盖 / 另存副本 / 追加合并（md/txt 文本）；文件内容仅在拉取时传输，走既有加密通道。
- **skills 文件夹整体递归拉取** — 勾选一个技能/命令文件夹即递归拉取其下全部文件，批量进度「N/M」实时显示，失败可单独重试。
- **一键迁移向导** — 选源设备 → 按工具分组的差异清单 → 一键应用（冲突默认跳过，可选覆盖留 .bak / 另存副本）。
- **新旧对比徽标** — 每行标注 缺失 / 相同 / 本地新 / 远端新（按哈希与修改时间比对），拉取前即可判断方向；旧版对端清单可只读浏览与预览。
- **本机配置管理器** — 不配对也能管理清单内文件：浏览/预览/编辑保存（自动留 .bak）、删除进回收站（不硬删）、打开所在文件夹；技能/命令等子文件夹可展开折叠。
- 注意：仅限已配对设备之间（跨设备浏览）；单个文件超过 1MB 会被截断并在界面上标注。
...
- 查看剪贴板历史，推送文字到电脑剪贴板
- 手机与电脑间上传下载文件
- 可堆叠 toast 通知；历史列表支持键盘导航（↑↓ 选择、Enter 复制、Del 删除）与 最新/最旧 排序切换；收藏可一键推送到电脑剪贴板；链接条目右键可直接在电脑浏览器打开
- Token 认证（自动生成或自定义）
- **全面诊断** — 诊断页分 7 组（系统 / 网络 / 互联网 / AI 配置 / 聊天 / 传输 / 文件系统），每项 ok/warn/fail 状态 + 详情 + 修复提示
- **测试中继连通性** — 设置→网络可一键测试每个中继服务器的可达性与延迟（TCP/TLS 握手，不干扰在线连接）

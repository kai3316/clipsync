...
class ClipboardHistory:
    ...
    # instead of stacking a second (possibly flavor-poorer) record.  Mirrors
    # the sync manager's DEDUP_RING_TTL: inside it, a manual re-copy of the
    # same text is the same logical item — merging keeps its richer flavors
    # (HTML/RTF) alive; past it, a re-copy is deliberate and starts a fresh
    # entry, as before.
        ...
        *source_app* is an optional dict with ``name``, ``process``, and
        ...
        with self._lock:
            if (dedup_key == self._last_dedup_key
                ...
                # (e.g. HTML written a beat after TEXT) must upgrade the
                # entry, not be dropped — that drop used to lose the rich
                # flavor forever.  A true duplicate stays dropped.

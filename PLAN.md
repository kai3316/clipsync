...
| **Phase 6** | 文件传输（分块/TCP直传/进度）、开机自启、PyInstaller 打包 | ✅ 完成 |
| **Phase 7** | 跨平台联调测试、压力测试、代码签名 | 🔄 联调/压力测试由 949 个测试覆盖；代码签名未做 |
| **Phase 8** | Web-first 化：经典 Tk 界面逐步弃用（仅崩溃级修复），新功能只加 Web 仪表盘；mobile.html 与桌面对齐 | ✅ 完成（v1.0.60） |
| **Phase 9** | 互联网同步：免费公共 MQTT/WebSocket 中继跨网、端到端加密、配对 SAS 安全码 | ✅ 完成（v1.0.61；中继仅承载剪贴板帧，大文件/聊天不走中继） |
| **Phase 10** | AI 配置同步：Web「配置」tab 浏览已配对设备监视清单，预览+三模式拉取，绝不静默覆盖 | ✅ 完成（v1.0.62） |
...
> **当前方向（v1.0.60 起）**：新功能只加 Web 界面；经典 Tk 界面逐步弃用，仅做崩溃级修复。下列"近期/中期/远期"为早期路线图，已实现项已标注。

### 近期（v1.1 - v1.3）

1. **开机自启**：Windows 注册表 / macOS LaunchAgent / Linux autostart ✅ 已实现
2. **PyInstaller 打包**：生成独立 .exe / .app / Linux 二进制，无需 Python 环境 ✅ 已实现
3. **通知提醒**：配对请求时桌面通知、同步失败提醒 ✅ 已实现
4. **剪贴板历史**：本地存储最近 50 条记录，支持搜索和重新复制 ✅ 已实现
5. **内容过滤**：检测密码/密钥特征，可选不同步敏感内容 ✅ 已实现
...
7. **中继服务器**：WebSocket 中继实现跨网络（不同 WiFi）同步，E2E 加密，中继不可见明文 ✅ 已实现（v1.0.61 互联网同步；中继仅承载剪贴板帧，大文件/聊天不走中继）
8. **二维码配对**：扫描二维码代替手动输入配对码 🔄 部分实现（扫码接入 Web 已支持；配对码扫码待后续）
9. **Web 管理面板**：localhost 上的 Web UI，查看连接状态和同步日志，支持全文本搜索和时间范围过滤 ✅ 已实现（Web 仪表盘）
...
### 剩余方向（按 .kai-round.md 交接记录）

- **轮 13 收口**：①Tk 配对对话框补 SAS 安全码展示；②Web 设置表单补 `ai_config_paths` 监视清单编辑；③核查 CI 上传 release 资产是否带 sha256 digest；④新表面回归抽查（relay×aiconfig 交互、config 新键往返）。
- **互联网同步后续**：大文件/聊天经中继；STUN 打洞为可选增强。已知限制：MVP 仅剪贴板帧走中继，桌面 Tk 配对框暂未显示 SAS。
- **AI 配置同步后续**：diff 合并、双向白名单自动同步、整包导出。
- **Web-first**：经典 Tk 界面仅崩溃级修复，不再加功能。

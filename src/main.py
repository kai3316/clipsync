...
def _detect_network_type() -> tuple[str, str]:
    ...
    Shared with the Tauri host — see ``internal.platform.network``.
    """
...
class Application:
    ...
    def _prepare_update_ready_archive(self, asset_path: str) -> None:
        ...
        """
        # The stash location and move live in the shared update service so the
        # native sidecar stages an archive in exactly the same place.
        from internal.system.update_service import stage_ready_archive

        try:
            dest = stage_ready_archive(asset_path)
        except Exception:
        ...
        self.root.after(0, lambda: self._notify_update_ready(version, dest))
    ...
    def _handle_diagnostics_request(self, action: str) -> dict:
        """Open the relevant OS permission/firewall settings, or repair a rule.

        Delegates to ``internal.diagnostics.actions`` so the legacy web panel
        and the native desktop request exactly the same repair.
        """
        from internal.diagnostics.actions import request

        return request(action, self.cfg)
    ...
    def _diag_fmt_duration(secs) -> str:
        """Legacy helper contract; see ``internal.diagnostics.report``."""
        from internal.diagnostics.report import fmt_duration
    ...
    def _diag_fmt_bytes(n) -> str:
        """Legacy helper contract; see ``internal.diagnostics.report``."""
        from internal.diagnostics.report import fmt_bytes

        return fmt_bytes(n)

    def _get_diagnostics(self) -> dict:
        """Live snapshot of core service state + actionable network checks.

        Built by ``internal.diagnostics.report`` so the web panel and the
        native desktop page can never disagree about the state of the machine.
        """
        ...
        return build_report(
            ...
            lan_ip=lan_ip,
            web=web_server,
            web_running=bool(web_server is not None and getattr(web_server, "is_running", False)),
            transport=getattr(self, "transport_mgr", None),
            discovery=getattr(self, "discovery", None),
            pairing=getattr(self, "pairing_mgr", None),
            chat=getattr(self, "chat_mgr", None),
            ai_config=getattr(self, "aicfg_mgr", None),
            file_transfer=getattr(self, "file_transfer_mgr", None),
            history=getattr(self, "clipboard_history", None),
            start_time=getattr(self, "_start_time", 0),
            relay_state=self._get_relay_state,
            delivery_counts=self._delivery_counts,
        )
    ...
    def _open_file(self, file_path: str) -> None:
        ...
        # The platform commands live in the shared helper so the native sidecar
        # opens a file exactly the same way.
        from internal.system.file_manager import FILE_NOT_FOUND, open_file

        ok, detail = open_file(file_path)
        if ok:
            return
        if detail == FILE_NOT_FOUND:
            ...
            self._notify_error(T("ui.open_failed_title"), T("ui.open_failed_msg", path=file_path))

    def _open_folder(self, file_path: str) -> None:
        """Open the containing folder in the OS file manager."""
        # The platform commands live in the shared helper so the native sidecar
        # reveals a folder exactly the same way.
        ...
        ok, detail = reveal_folder(file_path)
        if ok:
            return
        if detail == FILE_NOT_FOUND:
            self._notify_error(
        ...
        elif detail == FOLDER_NOT_FOUND:
            self._notify_error(
                T("ui.folder_not_found_title"), T("ui.folder_not_found_msg", path=file_path)
            )
        else:
            logger.error("Failed to open folder for %s", file_path)
            self._notify_error(
                T("ui.open_failed_title"), T("ui.open_failed_msg", path=file_path)
            )
...
def main():
    ...
    # Shared with the Tauri sidecar so new and legacy launchers cannot write
    # the same data concurrently. The existing PID lock still owns tray cleanup.
    data_lock = InstanceLock(_config_dir(), check_legacy=False)
    try:
        data_lock.start()
    except DataInUseError:
        logger.error("ClipSync data directory is already in use")
        return
    atexit.register(data_lock.stop)

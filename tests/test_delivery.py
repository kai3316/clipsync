"""Internet delivery — the ``relay_ack`` receipt and the offline retransmission queue.


The relay path is QoS0 best-effort: no receipt, offline is loss.  This file
covers the delivery ledger (sent → ack → delivered, or → failed on ack
timeout), the persisted offline queue flushed on reconnect / peer-active /
timer, and the ``relay_ack`` receipt published back on the same encrypted relay
channel when a clipboard frame lands in the receiver's history.  Chat frames
ride the same ledger but are never queued offline.

Handlers are exercised through the make_app_stub pattern (as in
tests/test_internet_pairing.py / tests/test_chat_api.py): no app / tkinter /
transport stack boots here, and every frame crosses the wire through the real
codec.
"""
